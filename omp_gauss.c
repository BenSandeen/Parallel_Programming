...
void gauss() {
  ...
  // parallelizes the inner loops, which are independent from each other
  #pragma omp parallel for private(multiplier,row,col) shared(A,B) num_threads(procs) schedule(dynamic)
    for (row = norm + 1; row < N; row++) { // looks at all rows below norm
        multiplier = A[row][norm] / A[norm][norm]; // get normalizing factor
        // loops through each col to complete operations on all elements of row
        for (col = norm; col < N; col++) {
          A[row][col] -= A[norm][col] * multiplier;
        }
        B[row] -= B[norm] * multiplier; // performs same ops on the B vector
      }
 }
